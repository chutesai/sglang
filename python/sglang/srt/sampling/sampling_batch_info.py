...
class SamplingBatchInfo:
    ...
    def apply_logits_bias(self, logits: torch.Tensor):
        ...
        if self.logit_bias is not None:
            if self.logit_bias.shape[0] == logits.shape[0]:
                logits.add_(self.logit_bias)
            else:
                logger.warning(
                    "logit_bias shape mismatch: logit_bias has %d rows but "
                    "logits has %d rows. Skipping logit_bias for this batch "
                    "to avoid misapplication.",
                    self.logit_bias.shape[0],
                    logits.shape[0],
                )
